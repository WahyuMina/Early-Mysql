...
from MsEmployee
where Gender = 'F'
limit 2; -- LImit berfung untuk membatasi Query nilainya
...
FROM MsEmployee
WHERE CHAR_LENGTH(EmployeeName) - CHAR_LENGTH(REPLACE(EmployeeName, ' ', '')) >= 2;  -- CARA PERTAMA
-- WHERE EmployeeName LIKE '% % %'; 	-- CARA KEDUA	
...
SELECT 
SUBSTRING_INDEX(MusicIns, ' ', 1) AS Brand,  -- Fungsi SUBSTRING_INDEX() mengembalikan sub-string dari sebuah string sebelum sejumlah pemisah yang ditentukan muncul .
...
WHERE EXISTS ( -- Exist befungsi pada befungsi menguji keberadaan baris dalam subquery tersebut!
    SELECT * 
